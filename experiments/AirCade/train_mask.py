...
def train(train_loader, model, optimizer, device, mask=None, optimizer_mask=None):
    model.train()
    train_loss = 0
    for batch_idx, data in tqdm(enumerate(train_loader)):
        ...
        for i in range(m.shape[0]):
            pm25_pred = model(torch.cat([pm25_hist, feature], dim=-1), feature1, m[i])
            loss.append(torch.fft.rfft(pm25_pred, dim=1) - torch.fft.rfft(pm25_label, dim=1)).abs().mean() 
        loss_mean = np.mean(loss)
        loss_var = np.var(loss)

        loss_model = loss_mean + loss_var
        loss_mask = log_p * loss_var.detach()
...
def main():
    ...
    for exp_idx in range(exp_repeat):
        ...
        mask = Mask(node_num_training)
        optimizer_mask = torch.optim.RMSprop(mask.parameters(), lr=lr, weight_decay=weight_decay)
        ...
        for epoch in range(epochs):
            ...
            train_loss = train(train_loader, model, optimizer, device, mask, optimizer_mask)
