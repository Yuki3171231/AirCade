...
class AirCade(BaseModel):
    ...
    def forward(self, x, y, m):
        ...
        for Cade_s, ES_s in zip(self.Inverse_transformer_encode, self.Inverse_transformer_deocde):
            x = Cade_s(x, y, True, dim=2, mask=m)
            y = ES_s(x, y, False, dim=2, mask=m)
...
class DK_MSA(nn.Module):
    ...
    def forward(self, query, key, value, dim, m):
        ...
        out_init =[]
        attn_score_row = torch.softmax(attn_score + m.log() if m is not None else 0, dim=-1)
        attn_score_col = torch.softmax(attn_score + m.log() if m is not None else 0, dim=-2)  
        out_init.append(attn_score_row @ value)
        out_init.append((attn_score_col).transpose(-1,-2) @ value) 
        adp_row = torch.softmax(self.node_emb1 @ self.node_emb2 + m.log() if m is not None else 0, dim=-1)
        adp_col = torch.softmax(self.node_emb1 @ self.node_emb2 + m.log() if m is not None else 0, dim=0)
        if dim == 2:
...
class Attention_Mechanism(nn.Module):
    ...
    def forward(self, x , y , flag , dim=1, m=None):  
        x = x.transpose(dim, 2) 
        y = y.transpose(dim, 2)
        residual = x
        if flag:
            out = self.Cade(x, x, x, dim, m)  
        else:
            residual = y
