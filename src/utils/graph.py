...
city_fp = os.path.join('data/city.txt')
altitude_fp = os.path.join('data/altitude.npy')
...
class Graph():
    def __init__(self):
        ...
        self.attr_adj_dist, self.attr_adj_direc = self._attr_adj()
        # self.attr_adj_dist = self._calculate_asym_adj(self.attr_adj_dist)
        # self.attr_adj_direc = self._calculate_asym_adj(self.attr_adj_direc)
    ...
    def _attr_adj(self):
        attr_adj_dist = np.zeros((self.node_num, self.node_num))
        attr_adj_direc = np.zeros((self.node_num, self.node_num))
        for i in range(self.edge_num):
            attr_adj_dist[self.edge_index[0, i], self.edge_index[1, i]] = self.edge_attr[i, 0]
            attr_adj_direc[self.edge_index[0, i], self.edge_index[1, i]] = self.edge_attr[i ,1]
        return attr_adj_dist, attr_adj_direc
    
    def _calculate_asym_adj(self, adj_mx):
        ...
        return [a.astype(np.float32).todense() for a in [res]][0]
